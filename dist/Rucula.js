...
class WindowBaseDOM {
    ...
    createMold(type, openLeftGrid = true) {
        var body = '';
        switch (type) {
            case 'crud':
                body = this.bodyForm();
    ...
    headerWithWindowName(type, openLeftGrid = false) {
    ...
    maximizeForm() {
        ...
        buttonMaximizeWindow?.addEventListener('click', (e) => {
            ...
            let icon = buttonMaximizeWindow?.querySelector('i');
    ...
    setEventClickQuerySelectorAll(eventName, selector, callback) {
    ...
    setEventClickById(eventName, id) {
        const evtName = `${this.P}${eventName}`;
        let evt = new Event(evtName);
...
function setCssClass(cssClass, input) {
...
class FieldInput {
    ...
    input;
    ruculaForm;
    constructor(p, field, managmentObject, ruculaForm) {
        this.field = field;
        this.managmentObject = managmentObject;
...
class FileEvent {
    ...
    ruculaForm;
    managmentObject;
    constructor(p, managmentObject, input, field, ruculaForm) {
        this.p = p;
        this.ruculaForm = ruculaForm;
        this.managmentObject = managmentObject;
        this.input = input;
...
class FileEventSelectFile extends FileEvent {
    setEventListener() {
        this.input.addEventListener('change', (e) => {
            this.dispatchEvent(constPrefixEventField.BEFORE);
            this.set();
...
class FieldFile extends FieldInput {
    create() {
        var input = document.createElement("input");
        this.input = input;
        this.input.classList.add('r-i-control');
...
class FileEventImage extends FileEvent {
    setEventListener() {
...
class FieldImage extends FieldInput {
    create() {
...
class Field {
    ...
    ruculaForm;
    p;
    constructor(p, managmentObject, ruculaForm) {
        this.managmentObject = managmentObject;
        this.ruculaForm = ruculaForm;
        this.p = p;
    }
...
class ButtonCrud {
    ...
    constructor(P, crud, globalwindow) {
        this.P = P;
        this.crud = crud;
        this.buttonCreate = globalwindow.querySelector(`#${this.P}${constTargetButtonCrudDefault.SAVE}`);
        this.buttonAlter = globalwindow.querySelector(`#${this.P}${constTargetButtonCrudDefault.ALTER}`);
        this.buttonDelete = globalwindow.querySelector(`#${this.P}${constTargetButtonCrudDefault.DELETE}`);
    }
    buttonCreate;
    buttonAlter;
    buttonDelete;
...
class Rucula {
    ...
    perspective;
    EN = eventsName;
    evInstance = eventIstance(this.P);
    constructor(config) {
